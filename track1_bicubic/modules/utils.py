...
from track1_bicubic.modules.dataset import load_tfrecord_dataset
...
def calculate_edge_loss(img1,img2):
    '''Define the soft edge loss'''
    if not img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    img1 = img1 / 255
    img2 = img2 / 255
    ...
    def _transform(u1, u2):
        base = np.sqrt(1 + np.square(u1) + np.square(u2))
        u1 = np.divide(u1, base)
        u2 = np.divide(u2, base)
        return u1, u2

    def _soft_edge(img):
        u1, u2 = np.gradient(img)
    ...
    for i in range(img1.shape[-1]):
        # mse += np.mean((_soft_edge(img1) - _soft_edge(img2)) ** 2)
        mae += np.mean(np.abs((_soft_edge(img1[:,:,i]) - _soft_edge(img2[:,:,i]))))
    # upscale the loss
    return mae*255/3
