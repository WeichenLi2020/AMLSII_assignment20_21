...
def RRDB_Model(size, channels, cfg_net, gc=32, wd=0., name='RRDB_model',scale=4):
    ...
    if scale == 2:
        fea_resize = tf.image.resize(fea, [size_fea_h * 2, size_fea_w * 2],
                                     method='nearest', name='upsample_x2_nn_1')
        fea = conv_f(filters=nf, activation=lrelu_f(), name='upsample_x2_conv_1')(fea_resize)
    elif scale == 3:
        fea_resize = tf.image.resize(fea, [size_fea_h * 3, size_fea_w * 3],
                                     method='nearest', name='upsample_x3_nn_1')
        fea = conv_f(filters=nf, activation=lrelu_f(), name='upsample_x3_conv_1')(fea_resize)
    elif scale == 4:
        fea_resize = tf.image.resize(fea, [size_fea_h * 2, size_fea_w * 2],
                                     method='nearest', name='upsample_x4_nn_1')
        fea = conv_f(filters=nf, activation=lrelu_f(), name='upsample_x4_conv_1')(fea_resize)
        fea_resize = tf.image.resize(fea, [size_fea_h * 4, size_fea_w * 4],
                                     method='nearest', name='upsample_x4_nn_2')
        fea = conv_f(filters=nf, activation=lrelu_f(), name='upsample_x4_conv_2')(fea_resize)
    # fea_resize = tf.image.resize(fea, [size_fea_h * 2, size_fea_w * 2],
...
def RRDB_Model_scale(size, channels, cfg_net, gc=32, wd=0.,scale=4, name='RRDB_model'):
    """Residual-in-Residual Dense Block based Model """
    nf, nb = cfg_net['nf'], cfg_net['nb']
    lrelu_f = functools.partial(LeakyReLU, alpha=0.2)
    rrdb_f = functools.partial(ResInResDenseBlock, nf=nf, gc=gc, wd=wd)
